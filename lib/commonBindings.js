...
const { withNormalizedMeasureTypes } = require('./normalizedMeasureTypes');
...
function sendMeasures(deviceId, device, values, callback) {
    const ctxt = fillService({ ...context }, device);
    const normalizedDevice = withNormalizedMeasureTypes(device, values);
    config.getLogger().debug(ctxt, 'Processing %d normalized measure(s) for device %s', values.length, deviceId);
    iotAgentLib.update(device.name, device.type, '', values, normalizedDevice, function (error) {
        if (error) {
