...
/**
 * Retrieve a device from the repository. MTEXNS MQTT topics contain a globally unique Device ID but no API key.
 * Existing devices are resolved globally. Unknown IDs remain unprovisioned until an operator assigns a service group.
 *
 * @param {String} deviceId         Device ID of the device that wants to be retrieved.
 * @param {String} apiKey           APIKey of the Device Group (or default APIKey).
 */
function retrieveDevice(deviceId, apiKey, callback) {
    if (!apiKey || apiKey === config.getConfig().defaultKey) {
        iotAgentLib.getDevicesByAttribute('id', deviceId, null, null, function (error, devices) {
            ...
            if (error) {
                ...
                config
                    ...
                    .error(
                        context,
                        'MEASURES-001: DeviceId %s is ambiguous: %d provisioned records were found. ' +
                            'MTEXNS MQTT Device IDs must be globally unique.',
                        deviceId,
                        matches.length
                    );
                callback(new errors.AmbiguousDevice(deviceId, matches.length));
            } else {
        ...
        async.waterfall(
            [
                ...
                apply(iotAgentLib.findOrCreate, deviceId, apiKey),
                apply(
