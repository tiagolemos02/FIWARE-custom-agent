...
const { createTelemetryQueue } = require('../telemetryQueue');
...
function warnLimited(key, message, ...args) {
    if (warningLimiter && warningLimiter.shouldLog(key)) {
        config.getLogger().warn(context, message, ...args);
    }
}

function isControlAttribute(attribute) {
    return ['config', 'cmd', constants.CONFIGURATION_COMMAND_UPDATE].includes(attribute);
}
...
function handleIncomingMessage(topic, message) {
    ...
    if (deviceId) deviceDiscovery.observe(deviceId);

    if (isControlAttribute(attribute)) {
        commonBindings.mqttMessageHandler(topic, message, function onControlProcessed(error) {
            if (error) {
                warnLimited(
                    `control:${deviceId}:${attribute}`,
                    'MQTT control message failed for device %s attribute %s: %s',
                    deviceId,
                    attribute,
                    error.message || error
                );
    ...
    if (messageByteLength(message) > mqttSetting('maxPayloadBytes', constants.MQTT_DEFAULT_MAX_PAYLOAD_BYTES)) {
        warnLimited(
            ...
            'MQTT payload was rejected for device %s attribute %s because it exceeds the configured size limit',
            deviceId,
            attribute
        );
        return;
    ...
    if (!telemetryQueue) {
        warnLimited('queue:not-ready', 'MQTT telemetry was rejected because the processing queue is not available');
        return;
    }
    ...
    telemetryQueue.enqueue({
        ...
        groupKey: deviceId,
        item: { topic, message, deviceId, attribute }
    });
}
...
function createProcessingQueue() {
    warningLimiter = createWarningLimiter({
        ...
        maxEntries: mqttSetting('limitCacheMaxEntries', constants.MQTT_DEFAULT_LIMIT_CACHE_MAX_ENTRIES)
    });

    telemetryQueue = createTelemetryQueue({
        concurrency: mqttSetting('updateConcurrency', constants.MQTT_DEFAULT_UPDATE_CONCURRENCY),
        maxPending: mqttSetting('maxPending', constants.MQTT_DEFAULT_MAX_PENDING),
        worker(item, done) {
            commonBindings.mqttMessageHandler(item.topic, item.message, function onTelemetryProcessed(error) {
                if (error) {
                    warnLimited(
                        `worker:${item.deviceId}:${item.attribute}`,
                        'MQTT telemetry processing failed for device %s attribute %s: %s',
                        item.deviceId,
                        item.attribute,
                        error.message || error
                    );
        ...
        onDrop(item, reason) {
            warnLimited(
                `drop:${item.deviceId}:${item.attribute}:${reason}`,
                'MQTT telemetry was dropped for device %s attribute %s (%s)',
                item.deviceId,
                item.attribute,
                reason
            );
    ...
    queueStatsTimer = setInterval(() => {
        const stats = telemetryQueue ? telemetryQueue.getStats() : null;
        if (stats) {
            config
                .getLogger()
                .debug(
                    context,
                    'MQTT telemetry queue stats: active=%d pending=%d enqueued=%d coalesced=%d completed=%d failed=%d dropped=%d',
                    stats.active,
                    stats.pending,
                    stats.enqueued,
                    stats.coalesced,
    ...
    }, mqttSetting('queueLogIntervalMs', constants.MQTT_DEFAULT_QUEUE_LOG_INTERVAL_MS));
    if (typeof queueStatsTimer.unref === 'function') {
...
function generateTopics(callback) {
    ...
    // <deviceId>/state/<attributes>
    //ex: 00:00:0A:B3:47:FA/state/ambient_humidity
    topics.push('+/state/+');
...
function sendConfigurationToDevice(apiKey, group, deviceId, results, callback) {
    ...
    // usar novo "MAC" + /state/config
    const leadingSlash = config.getConfig().mqtt.avoidLeadingSlash ? '' : '/';
    const commandTopic = leadingSlash + deviceId + '/state/config';

    if (mqttClient.connected) {
        mqttClient.publish(commandTopic, JSON.stringify(configurations), options, (error) => {
            if (error) {
                config
                    .getLogger()
                    .error(
...
function start(callback) {
    ...
    const mqttConfig = config.getConfig().mqtt;
    if (!mqttConfig) {
        config.getLogger().error(context, 'Error MQTT is not configured');
        return callback(new Error('MQTT is not configured'));
    }
    if (mqttConfig.disabled) {
        config.getLogger().warn(context, 'MQTT is disabled');
        return callback();
    }
    ...
    config
        ...
        .info(
            context,
            'Starting MQTT binding for %s://%s:%s with client %s, retries %s and retryTime %s',
            options.protocol,
            options.host,
            options.port,
            options.clientId,
...
function stop(callback) {
    ...
    function closeClient() {
        ...
        async.series([unsubscribeAll, client.end.bind(client, true)], function () {
            mqttClient = null;
            config.getLogger().info('MQTT Binding Stopped');
            callback();
        });
    ...
    if (telemetryQueue) {
        telemetryQueue.close(mqttSetting('shutdownTimeoutMs', constants.MQTT_DEFAULT_SHUTDOWN_TIMEOUT_MS), closeClient);
    } else {
        closeClient();
    }
}
...
function executeCommand(apiKey, group, device, cmdName, serializedPayload, contentType, callback) {
    ...
    options.qos =
        commands[cmdName] && commands[cmdName].mqtt && commands[cmdName].mqtt.qos
            ? parseInt(commands[cmdName].mqtt.qos)
            : config.getConfig().mqtt.qos
            ? parseInt(config.getConfig().mqtt.qos)
            : 0;
    options.retain =
        commands[cmdName] && commands[cmdName].mqtt && commands[cmdName].mqtt.retain
            ? commands[cmdName].mqtt.retain
            : config.getConfig().mqtt.retain
            ? config.getConfig().mqtt.retain
            : false;

    const commandTopic = device.id + '/state/cmd';
    config
        .getLogger()
        .debug(
            context,
