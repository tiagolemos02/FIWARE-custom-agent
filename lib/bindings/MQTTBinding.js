...
function handleIncomingMessage(topic, message) {
    ...
    if (!telemetryQueue) {
        warnLimited(
            'queue:not-ready',
            'MQTT telemetry was rejected because the processing queue is not available'
        );
        return;
    }
...
function createProcessingQueue() {
    ...
    if (typeof queueStatsTimer.unref === 'function') queueStatsTimer.unref();
}
...
function stop(callback) {
    ...
    if (telemetryQueue) {
        telemetryQueue.close(
            mqttSetting('shutdownTimeoutMs', constants.MQTT_DEFAULT_SHUTDOWN_TIMEOUT_MS),
            closeClient
        );
    } else {
        closeClient();
