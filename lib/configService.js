...
function readPositiveIntegerEnvironment(key) {
    if (process.env[key] === undefined) return undefined;
    const value = Number.parseInt(process.env[key], 10);
    if (!Number.isFinite(value) || value <= 0) {
        logger.warn('Ignoring invalid positive integer environment value for %s.', key);
        return undefined;
    }
    return value;
}
...
function processEnvironmentVariables() {
    const environmentVariables = [
        ...
        'IOTA_MQTT_CLIENT_ID',
        'IOTA_MQTT_DISABLED',
        'IOTA_MQTT_UPDATE_CONCURRENCY',
        'IOTA_MQTT_MAX_PENDING',
        'IOTA_MQTT_MAX_PAYLOAD_BYTES',
        'IOTA_MQTT_LIMIT_CACHE_MAX_ENTRIES',
        'IOTA_MQTT_WARNING_INTERVAL_MS',
        'IOTA_MQTT_QUEUE_LOG_INTERVAL_MS',
        'IOTA_MQTT_SHUTDOWN_TIMEOUT_MS',
        'IOTA_MQTT_DISCOVERY_TTL_MS',
        'IOTA_MQTT_DISCOVERY_MAX_ENTRIES',
        'IOTA_AMQP_HOST',
        'IOTA_AMQP_PORT',
        'IOTA_AMQP_USERNAME',
        'IOTA_AMQP_PASSWORD',
    ...
    const mqttIntegerSettings = [
        ['IOTA_MQTT_UPDATE_CONCURRENCY', 'updateConcurrency'],
        ['IOTA_MQTT_MAX_PENDING', 'maxPending'],
        ['IOTA_MQTT_MAX_PAYLOAD_BYTES', 'maxPayloadBytes'],
        ['IOTA_MQTT_LIMIT_CACHE_MAX_ENTRIES', 'limitCacheMaxEntries'],
        ['IOTA_MQTT_WARNING_INTERVAL_MS', 'warningIntervalMs'],
        ['IOTA_MQTT_QUEUE_LOG_INTERVAL_MS', 'queueLogIntervalMs'],
        ['IOTA_MQTT_SHUTDOWN_TIMEOUT_MS', 'shutdownTimeoutMs'],
        ['IOTA_MQTT_DISCOVERY_TTL_MS', 'discoveryTtlMs'],
        ['IOTA_MQTT_DISCOVERY_MAX_ENTRIES', 'discoveryMaxEntries']
    ];
    mqttIntegerSettings.forEach(([environmentKey, configKey]) => {
        const value = readPositiveIntegerEnvironment(environmentKey);
        if (value !== undefined) config.mqtt[configKey] = value;
    });
